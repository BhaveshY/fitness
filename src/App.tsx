...
function ProtectedRoute({ children }: { children: React.ReactNode }) {
  const { user, isAuthReady } = useFitness();
...
function Login() {
  const { user, signIn, isAuthReady } = useFitness();
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50">
      <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 text-center max-w-md w-full">
        <h1 className="text-2xl font-bold text-slate-900 mb-2">Welcome to FitSync AI</h1>
        <p className="text-slate-500 mb-8">Sign in to sync your fitness journey across all your devices.</p>
        <button
          onClick={signIn}
